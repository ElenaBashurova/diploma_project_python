...
class PageProduct:
    ...
    def search_product_enter(self, product_name):
        ...
        with allure.step('Товар найден на странице'):
            browser.element('.ProductHead__name').should(
                have.text('Кресло BRABIX «Tender MG-330», с подлокотниками, хром, черное, 531845'))
        return self
    ...
    def search_part_product(self, part_name):
        ...
        with allure.step('Товар найден на странице'):
            browser.element('.ProductHead__name').should(
                have.text('Шкаф (стеллаж) «Канц», 700×330×1830 мм, 4 полки, цвет бук невский, ШК31.10'))
        return self
